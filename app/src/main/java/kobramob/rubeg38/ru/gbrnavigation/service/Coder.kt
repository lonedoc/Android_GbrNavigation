...
class Coder {
    ...
    private val sizeHeading = 39

    fun decoderOne(message: String, count: Long, typePacket: Byte): ByteBuffer {
        ...
        xor(heading, typePacket, packetSize, 0, 0, positionOnMassive, allSize, count, allPacket, TPacket)
        val headingArray = heading.array()
        coderZag(headingArray, mass1)
        heading.position(0)
        heading.put(headingArray)
        ...
        packet.position(0)
        packet.put(packetArray)
        // Соединили
        val request: ByteBuffer = ByteBuffer.allocate(sizeFullPacket)
        request.order(ByteOrder.LITTLE_ENDIAN)
        request.put(heading.array())
        request.put(packet.array())
        return request
    }
    fun encoderPacketOne(message: ByteArray): String {
        // Енкодим заголовок
        val headingArray: ByteArray = ByteArray(41)
        System.arraycopy(message, 0, headingArray, 0, headingArray.size)
        coderZag(headingArray, mass1)
        // Узнаем размер пакета, енкодим пакет
    ...
    private fun packetString(message: ByteArray, lengthPacket: Int): String {
        ...
        System.arraycopy(message, 41, packetArray, 0, lengthPacket)
    ...
    private fun calculateSize(heading: ByteArray): Int {
        val byteBuffer: ByteBuffer = ByteBuffer.allocate(41)
    ...
    fun packetType255(message: ByteArray): ByteBuffer {
        var headingArray: ByteArray = ByteArray(41)
        System.arraycopy(message, 0, headingArray, 0, headingArray.size)
        coderZag(headingArray, mass1)
        ...
        byteBuffer.clear()
        xor(byteBuffer, typePacket, 0, sizeFirst, sizeSecont, positionOnMassive, allSize, count, allPacket, tPacket)
